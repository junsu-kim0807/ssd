...
class ModelRunner:
    ...
    def copy_kv_blocks(
        ...
        # Mode: ``bucketed_partial`` (default) groups by valid_tokens and emits
        # one fancy-indexed scatter per bucket (N→1-2 kernel launches).
        # ``full_block`` issues a single full-block scatter regardless of
        # partial lengths — trades extra bytes for a guaranteed single launch.
        # dst blocks are freshly allocated for the branch and any
        # ``[partial_tokens, block_size)`` tail is overwritten by the branch's
        # subsequent rollout, so overcopying parent's stale KV there is inert.
        # Env var is per cache_kind so target/intermediate can be tuned
        # independently of the draft path.
        if cache_kind == "target":
        ...
        if mode == "full_block":
            ...
            if srcs:
                ...
                if len(srcs) == 1:
                    kv_cache[:, :, dsts[0]] = kv_cache[:, :, srcs[0]]
                else:
            ...
            # bucket. In practice all full prefix blocks share v=block_size and
            # at most one residual partial block has a smaller v, so N kernel
            # launches collapse into 1-2.
